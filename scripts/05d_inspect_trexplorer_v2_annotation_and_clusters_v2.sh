...
python - "$JSON_GZ" "$JSON_PREVIEW" <<'PY'
import gzip
import sys
...
with gzip.open(input_path, "rt", encoding="utf-8") as handle:
    prefix = handle.read(1200)

Path(output_path).write_text(prefix, encoding="utf-8")
print(prefix)
PY
...
def extract_one_value(stream, first_char: str) -> str:
    ...
    escaped = False
    depth = 1 if first_char in "[{" else 0
...
with gzip.open(input_path, "rt", encoding="utf-8") as handle:
    ...
    if top == "[":
        ...
        raise RuntimeError(f"Unsupported top-level JSON token: {top!r}")
...
gzip -cd "$CLUSTER_GZ" |
awk 'NR <= 10 {print} NR == 10 {exit}' |
tee "$CLUSTER_FIRST" || true
