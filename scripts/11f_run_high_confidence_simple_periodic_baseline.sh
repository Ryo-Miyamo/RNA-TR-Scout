...
OUTDIR="$PROJECT_ROOT/results/11_periodic_baseline/$RUN_ID"
QCDIR="$PROJECT_ROOT/qc/11_periodic_baseline/$RUN_ID"
WORKDIR="$PROJECT_ROOT/tmp/11_periodic_baseline/$RUN_ID"
...
echo "===== MOST COMMON MOTIFS ====="
{
    ...
    tail -n +2 "$MOTIF_SUMMARY" | head -n 30
} |
column -ts $'\t'
