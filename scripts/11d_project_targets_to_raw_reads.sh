...
PARAMETER_SET_ID="rnatr_raw_projection_v0.3.1"
...
OUTDIR="$PROJECT_ROOT/results/11_projection/$RUN_ID"
QCDIR="$PROJECT_ROOT/qc/11_projection/$RUN_ID"
WORKDIR="$PROJECT_ROOT/tmp/11_projection/$RUN_ID"

DATA_OUTDIR="$RAW_ROOT/benchmarks/ENCSR307SHM/pilot_100k_seed20260803/rnatr_projection_v0.3.1"

PROJECTION="$OUTDIR/read_target_projection.tsv.gz"
WINDOW_FASTQ="$DATA_OUTDIR/ENCFF260PGB.pilot_100k.rnatr_target_windows.fastq.gz"
QC_SUMMARY="$QCDIR/raw_projection_qc.tsv"
PARAMETERS="$OUTDIR/${PARAMETER_SET_ID}.parameters.tsv"
MANIFEST="$OUTDIR/${RUN_ID}.raw_projection_manifest.tsv"
...
def convert_bam_query_interval_to_raw(
    start: int,
    end: int,
    read_length: int,
    is_reverse: bool,
...
with pysam.AlignmentFile(bam_path, "rb") as bam, gzip.open(
    ...
    for record in bam.fetch(until_eof=True):
        ...
        if record.query_sequence:
            ...
            if forward_sequence == raw_sequence:
                orientation_match += 1
            else:
        ...
        for candidate in candidate_list:
            ...
            if selected_block is None:
                ...
                if query_intervals:
                    ...
                    projected_raw = (
                        convert_bam_query_interval_to_raw(
                            projected_bam_start,
                            projected_bam_end,
                            read_length,
                            record.is_reverse,
                ...
                if projected_raw is not None:
                    ...
                    bam_window = (
                        ...
                        min(
                            read_length,
                            bam_query_start + sequence_flank,
                        ),
                    )
                    window_raw = (
                        convert_bam_query_interval_to_raw(
            ...
            if orientation_mismatch:
                # Per-record mismatches are rare and are audited globally.
                pass
