...
PARAMETER_SET_ID="rnatr_raw_projection_v0.3.2"
...
OUTDIR="$PROJECT_ROOT/results/11_projection/$RUN_ID/v0.3.2"
QCDIR="$PROJECT_ROOT/qc/11_projection/$RUN_ID/v0.3.2"
WORKDIR="$PROJECT_ROOT/tmp/11_projection/$RUN_ID/v0.3.2"

DATA_OUTDIR="$RAW_ROOT/benchmarks/ENCSR307SHM/pilot_100k_seed20260803/rnatr_projection_v0.3.2"

PROJECTION="$OUTDIR/read_target_projection.v0.3.2.tsv.gz"
WINDOW_FASTQ="$DATA_OUTDIR/ENCFF260PGB.pilot_100k.rnatr_target_windows.v0.3.2.fastq.gz"
QC_SUMMARY="$QCDIR/raw_projection_qc.v0.3.2.tsv"
PARAMETERS="$OUTDIR/${PARAMETER_SET_ID}.parameters.tsv"
MANIFEST="$OUTDIR/${RUN_ID}.raw_projection_manifest.v0.3.2.tsv"
...
sequence_audit	hardclip_aware_raw_slice	BAM forward sequence is compared with the corresponding non-hard-clipped raw-read slice
supersedes	rnatr_raw_projection_v0.3.1	v0.3.1 treated hard-clipped supplementary BAM sequence as a full raw read
...
with pysam.AlignmentFile(bam_path, "rb") as bam, gzip.open(
    ...
    for record in bam.fetch(until_eof=True):
        ...
        present_query_length = record.query_length or 0
        reconstructed_full_query_length = (
            hardclip_left_cigar
            + present_query_length
            + hardclip_right_cigar
        )
