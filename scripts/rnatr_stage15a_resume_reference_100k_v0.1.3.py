...
V012_SHADOW = V012_RESULT_ROOT / "shadow_project.work"
...
ACTIVE_11F = (
    PROJECT_ROOT
    / "scripts/11f_run_high_confidence_simple_periodic_baseline.parallel_v0.1.0.sh"
)
ACTIVE_11H = (
    PROJECT_ROOT
    / "scripts/11h_target_constrained_periodic_refinement.parallel_v0.1.0.sh"
)
ACTIVE_11F_EXPECTED_SHA256 = (
    "08d70a104ec384914a9e7e72cc18b67481b94805940fb67900d19c8fde397684"
)
ACTIVE_11H_EXPECTED_SHA256 = (
    "bad281accad9937429f450e538c657ae04e1090eba05157a0c911b375b82c7e0"
)
...
def patch_caller_driver(destination: Path) -> None:
    """Freeze the promoted Stage14K driver with Stage15A path plumbing only.

    The Stage14 driver consumes the historical target-constrained periodic
    prior.  Stage15A v0.1.3 now produces that prior freshly through 11f and
    11h inside the shadow root; it never links the historical 11h result into
    the candidate run.
    """
    ensure_file(HOST_CALLER_DRIVER)
    original = HOST_CALLER_DRIVER.read_text(encoding="utf-8")
    if "target_constrained_periodic_calls.tsv.gz" not in original:
        raise RuntimeError(
            "promoted caller driver no longer declares the 11h prior input"
        )
    patched = original
    replacements = {
        ...
        str(PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID): str(
            SHADOW / "results/11_periodic_baseline" / RUN_ID
        ),
        str(PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID): str(
            SHADOW / "results/11_periodic_refinement" / RUN_ID
        ),
    }
    replacement_counts: dict[str, int] = {}
    for old, new in replacements.items():
        replacement_counts[old] = patched.count(old)
    ...
    contract_path = CONTRACT_OUT / "stage15a_v013_caller_path_resolution.tsv"
    with contract_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle, delimiter="\t", lineterminator="\n")
        writer.writerow(["source", "replacement", "occurrences_in_source"])
        for old, new in replacements.items():
            writer.writerow([old, new, replacement_counts[old]])
    if str(ORIGINAL_RAW_ROOT) in patched:
        raise RuntimeError("caller driver still contains the active raw root")


def verify_active_11f_11h() -> Path:
    rows: list[dict[str, object]] = []
    for role, path, expected in [
        ("ACTIVE_11F", ACTIVE_11F, ACTIVE_11F_EXPECTED_SHA256),
        ("ACTIVE_11H", ACTIVE_11H, ACTIVE_11H_EXPECTED_SHA256),
    ]:
        ensure_file(path)
        observed = sha256_file(path)
        status = "PASS" if observed == expected else "FAIL"
        ...
        if status != "PASS":
            raise RuntimeError(f"{role} SHA mismatch: {path}")
    path = CONTRACT_OUT / "stage15a_v013_11f_11h_source.audit.tsv"
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle,
...
def snapshot_active_periodic_outputs(label: str) -> Path:
    artifacts = [
        ("11f_calls", PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / "high_confidence_simple_periodic_calls.tsv.gz", "tsv"),
        ("11f_top500", PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / "high_confidence_simple_periodic_calls.top500.tsv", "tsv"),
        ("11f_motif_summary", PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / "high_confidence_simple_periodic_motif_summary.tsv", "tsv"),
        ("11f_qc", PROJECT_ROOT / "qc/11_periodic_baseline" / RUN_ID / "high_confidence_simple_periodic_qc.tsv", "tsv"),
        ("11f_parameters", PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / "rnatr_periodic_baseline_v0.3.1.parameters.tsv", "tsv"),
        ("11h_calls", PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / "target_constrained_periodic_calls.tsv.gz", "tsv"),
        ("11h_top500", PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / "target_constrained_periodic_calls.top500.tsv", "tsv"),
        ("11h_evidence_summary", PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / "target_constrained_evidence_summary.tsv", "tsv"),
        ("11h_qc", PROJECT_ROOT / "qc/11_periodic_refinement" / RUN_ID / "target_constrained_periodic_qc.tsv", "tsv"),
        ("11h_parameters", PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / "rnatr_target_constrained_periodic_v0.3.2.parameters.tsv", "tsv"),
    ]
    rows: list[dict[str, object]] = []
    for role, path, kind in artifacts:
        ensure_file(path)
        rows.append(
            {
                "role": role,
                "path": str(path),
                "bytes": path.stat().st_size,
                "rows": data_rows(path, kind),
                "raw_sha256": sha256_file(path),
                "logical_sha256": logical_sha256(path),
            }
        )
    out = CONTRACT_OUT / f"stage15a_v013_active_periodic_outputs_{label}.audit.tsv"
    ...
    with out.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerows(rows)
    if label == "after":
        before = CONTRACT_OUT / "stage15a_v013_active_periodic_outputs_before.audit.tsv"
        ensure_file(before)
        with before.open("r", encoding="utf-8", newline="") as handle:
            before_rows = {
                row["role"]: row
                for row in csv.DictReader(handle, delimiter="\t")
            }
        for row in rows:
            prior = before_rows.get(str(row["role"]))
            if prior is None:
                raise RuntimeError(f"active periodic before-snapshot lacks {row['role']}")
            for field in ["path", "bytes", "rows", "raw_sha256", "logical_sha256"]:
                if str(row[field]) != str(prior[field]):
                    raise RuntimeError(
                        f"active periodic output changed during isolated run: {row['role']}:{field}"
                    )
    return out


def comparison_file_pass(path: Path) -> None:
    ensure_file(path)
    with path.open("r", encoding="utf-8", newline="") as handle:
    ...
    failures = []
    for row in rows:
        if row.get("logical_equal") != "true":
            failures.append(row.get("role", "."))
        if row.get("candidate_rows") != row.get("reference_rows"):
            failures.append(row.get("role", ".") + ":rows")
        if row.get("header_equal") not in {"true", "."}:
            failures.append(row.get("role", ".") + ":header")
    if failures:
        raise RuntimeError(
            f"v0.1.2 upstream comparison is not PASS: {path}: {failures[:10]}"
        )


def verify_v012_resume_source() -> list[dict[str, object]]:
    """Verify every reused upstream byte and preserve its measured runtime."""
    failure = V012_QC_ROOT / "stage15a_reference_100k.failure.txt"
    caller_log = V012_QC_ROOT / "logs/15A4_caller_production.log"
    ensure_file(failure)
    ensure_file(caller_log)
    failure_text = failure.read_text(encoding="utf-8", errors="replace")
    caller_log_text = caller_log.read_text(encoding="utf-8", errors="replace")
    if "stage 15A4_caller_production failed with exit 1" not in failure_text:
        raise RuntimeError("v0.1.2 failure record is not the expected caller failure")
    required_failure_fragment = (
        "missing input: "
        + str(
            V012_SHADOW
    ...
    if required_failure_fragment not in caller_log_text:
        raise RuntimeError(
            "v0.1.2 caller log is not the expected missing-11h closure failure"
        )

    stage_specs = [
        ("15A1_11b", "15A1_11b_comparison.tsv"),
        ("15A2_11d3", "15A2_11d3_comparison.tsv"),
        ("15A3_11e", "15A3_11e_comparison.tsv"),
    ]
    audit_rows: list[dict[str, object]] = []
    timing_records: list[dict[str, object]] = []
    for stage, comparison_name in stage_specs:
        marker_path = V012_QC_ROOT / "markers" / f"{stage}.done.json"
        ensure_file(marker_path)
        marker = json.loads(marker_path.read_text(encoding="utf-8"))
        if marker.get("stage") != stage:
            raise RuntimeError(f"v0.1.2 marker stage mismatch: {marker_path}")
        if marker.get("stage_version") != "rnatr_stage15a_reference_100k_v0.1.2":
            raise RuntimeError(f"v0.1.2 marker version mismatch: {marker_path}")
        if marker.get("production_component") is not True:
            raise RuntimeError(f"v0.1.2 marker is not a production component: {stage}")
        outputs = marker.get("outputs", [])
        if not outputs:
            raise RuntimeError(f"v0.1.2 marker has no outputs: {stage}")
        for entry in outputs:
            path = Path(entry["path"])
            assert_under(path, V012_RESULT_ROOT)
            if path.is_symlink():
                raise RuntimeError(f"v0.1.2 marker output unexpectedly became a symlink: {path}")
            ensure_file(path)
            if path.stat().st_size != int(entry["bytes"]):
                raise RuntimeError(f"v0.1.2 output size changed: {path}")
            observed_sha = sha256_file(path)
            if observed_sha != entry["sha256"]:
                raise RuntimeError(f"v0.1.2 output SHA changed: {path}")
        comparison_path = V012_QC_ROOT / "comparison" / comparison_name
        comparison_file_pass(comparison_path)
        elapsed = float(marker["elapsed_seconds"])
        timing_records.append(
            {
                "stage": stage,
                "stage_version": STAGE_VERSION,
                "source_stage_version": marker["stage_version"],
                "execution_mode": "VERIFIED_REUSE_V012",
                "started_utc": marker.get("started_utc", "."),
                "finished_utc": marker.get("finished_utc", "."),
                "command": ["VERIFIED_REUSE", str(marker_path)],
                "elapsed_seconds": elapsed,
                "maximum_resident_set_kbytes": marker.get(
        ...
        audit_rows.append(
            {
                ...
                "elapsed_seconds_carried_forward": f"{elapsed:.9f}",
                "status": "PASS_VERIFIED_REUSE",
            }
        )

    out = CONTRACT_OUT / "stage15a_v013_v012_resume_source.audit.tsv"
    with out.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(
            handle,
...
def setup_isolated_root() -> dict[str, Path]:
    ...
    # Reuse only v0.1.2 upstream artifacts that were just verified byte-for-byte.
    for branch in [
        ("results/11_assignment", "results/11_assignment"),
        ("qc/11_assignment", "qc/11_assignment"),
        ("results/11_projection", "results/11_projection"),
        ("qc/11_projection", "qc/11_projection"),
        ("results/11_motif_jobs", "results/11_motif_jobs"),
        ("qc/11_motif_jobs", "qc/11_motif_jobs"),
    ]:
        link_base, target_base = branch
        ensure_symlink(
            SHADOW / link_base / RUN_ID,
            V012_SHADOW / target_base / RUN_ID,
        )
    ...
    scripts = {
        "11f": FROZEN_SCRIPTS
        / "11f_run_high_confidence_simple_periodic_baseline.stage15a_v0.1.3.sh",
        "11h": FROZEN_SCRIPTS
        / "11h_target_constrained_periodic_refinement.stage15a_v0.1.3.sh",
        "caller": FROZEN_SCRIPTS / "run_native_v041_100k.stage15a_v0.1.3.py",
    }
    patch_upstream_script(ACTIVE_11F, scripts["11f"], shadow_env)
    patch_upstream_script(ACTIVE_11H, scripts["11h"], shadow_env)
    ...
    with script_ledger.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle, delimiter="\t", lineterminator="\n")
        writer.writerow(["stage", "source", "source_sha256", "path", "bytes", "sha256"])
        for stage, path in scripts.items():
            source = ACTIVE_11F if stage == "11f" else ACTIVE_11H if stage == "11h" else HOST_CALLER_DRIVER
            writer.writerow(
                [
                    stage,
                    source,
...
def run_logged(
    ...
    *,
    ...
    record: dict[str, object] = {
        "stage": name,
        "stage_version": STAGE_VERSION,
        "source_stage_version": STAGE_VERSION,
        "execution_mode": "EXECUTED_V013",
...
def collect_small_artifacts(output_tar: Path) -> None:
    ...
    small_paths = [
        ...
        SHADOW / "results/11_periodic_baseline" / RUN_ID / f"{RUN_ID}.periodic_baseline_manifest.tsv",
        SHADOW / "results/11_periodic_baseline" / RUN_ID / "rnatr_periodic_baseline_v0.3.1.parameters.tsv",
        SHADOW / "qc/11_periodic_baseline" / RUN_ID / "high_confidence_simple_periodic_qc.tsv",
        SHADOW / "results/11_periodic_refinement" / RUN_ID / f"{RUN_ID}.target_constrained_periodic_manifest.tsv",
        SHADOW / "results/11_periodic_refinement" / RUN_ID / "rnatr_target_constrained_periodic_v0.3.2.parameters.tsv",
        SHADOW / "qc/11_periodic_refinement" / RUN_ID / "target_constrained_periodic_qc.tsv",
...
def main() -> int:
    ...
    print("resume_source\tVERIFIED_V0.1.2_11B_11D3_11E")
    print("amended_graph\t11b>11d3>11e>11f>11h>native_caller>schema_v0.4.2")
    print("historical_11h_reuse\tPROHIBITED")
    ...
    verify_active_11f_11h()
    snapshot_active_periodic_outputs("before")
    timing_records = verify_v012_resume_source()
    scripts = setup_isolated_root()

    # Freshly recheck the three linked upstream branches against active frozen references.
    ...
    require_comparisons(compare_11b, "15A1/11b reused source")
    ...
    require_comparisons(compare_11d3, "15A2/11d3 reused source")
    ...
    require_comparisons(compare_11e, "15A3/11e reused source")

    baseline_dir = SHADOW / "results/11_periodic_baseline" / RUN_ID
    baseline_qc = SHADOW / "qc/11_periodic_baseline" / RUN_ID
    baseline_work = SHADOW / "tmp/11_periodic_baseline" / RUN_ID
    outputs_11f = [
        baseline_dir / "high_confidence_simple_periodic_calls.tsv.gz",
        baseline_dir / "high_confidence_simple_periodic_calls.top500.tsv",
        baseline_dir / "high_confidence_simple_periodic_motif_summary.tsv",
        baseline_qc / "high_confidence_simple_periodic_qc.tsv",
        baseline_dir / "rnatr_periodic_baseline_v0.3.1.parameters.tsv",
        baseline_dir / f"{RUN_ID}.periodic_baseline_manifest.tsv",
    ]
    timing_records.append(
        run_logged(
            "15A4_11f_periodic_baseline",
            ["bash", str(scripts["11f"])],
            outputs_11f,
            env={
                "RNATR_WORKERS": str(args.workers),
                "RNATR_11F_OUTDIR": str(baseline_dir),
                "RNATR_11F_QCDIR": str(baseline_qc),
                "RNATR_11F_WORKDIR": str(baseline_work),
            },
            production_component=True,
        )
    )
    compare_11f = [
        compare_pair(
            "high_confidence_simple_periodic_calls",
            outputs_11f[0],
            PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / outputs_11f[0].name,
        ),
        compare_pair(
            "high_confidence_simple_periodic_calls_top500",
            outputs_11f[1],
            PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / outputs_11f[1].name,
        ),
        compare_pair(
            "high_confidence_simple_periodic_motif_summary",
            outputs_11f[2],
            PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / outputs_11f[2].name,
        ),
        compare_pair(
            "high_confidence_simple_periodic_qc",
            outputs_11f[3],
            PROJECT_ROOT / "qc/11_periodic_baseline" / RUN_ID / outputs_11f[3].name,
        ),
        compare_pair(
            "rnatr_periodic_baseline_parameters",
            outputs_11f[4],
            PROJECT_ROOT / "results/11_periodic_baseline" / RUN_ID / outputs_11f[4].name,
        ),
    ]
    write_comparison(COMPARISON_DIR / "15A4_11f_comparison.tsv", compare_11f)
    require_comparisons(compare_11f, "15A4/11f")
    if data_rows(outputs_11f[0], "tsv") != 49793:
        raise RuntimeError("11f expected 49,793 rows")

    refinement_dir = SHADOW / "results/11_periodic_refinement" / RUN_ID
    refinement_qc = SHADOW / "qc/11_periodic_refinement" / RUN_ID
    refinement_work = SHADOW / "tmp/11_periodic_refinement" / RUN_ID
    outputs_11h = [
        refinement_dir / "target_constrained_periodic_calls.tsv.gz",
        refinement_dir / "target_constrained_periodic_calls.top500.tsv",
        refinement_dir / "target_constrained_evidence_summary.tsv",
        refinement_qc / "target_constrained_periodic_qc.tsv",
        refinement_dir / "rnatr_target_constrained_periodic_v0.3.2.parameters.tsv",
        refinement_dir / f"{RUN_ID}.target_constrained_periodic_manifest.tsv",
    ]
    timing_records.append(
        run_logged(
            "15A5_11h_periodic_refinement",
            ["bash", str(scripts["11h"])],
            outputs_11h,
            env={
                "RNATR_WORKERS": str(args.workers),
                "RNATR_11F_CALLS": str(outputs_11f[0]),
                "RNATR_11H_OUTDIR": str(refinement_dir),
                "RNATR_11H_QCDIR": str(refinement_qc),
                "RNATR_11H_WORKDIR": str(refinement_work),
            },
            production_component=True,
        )
    )
    compare_11h = [
        compare_pair(
            "target_constrained_periodic_calls",
            outputs_11h[0],
            PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / outputs_11h[0].name,
        ),
        compare_pair(
            "target_constrained_periodic_calls_top500",
            outputs_11h[1],
            PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / outputs_11h[1].name,
        ),
        compare_pair(
            "target_constrained_evidence_summary",
            outputs_11h[2],
            PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / outputs_11h[2].name,
        ),
        compare_pair(
            "target_constrained_periodic_qc",
            outputs_11h[3],
            PROJECT_ROOT / "qc/11_periodic_refinement" / RUN_ID / outputs_11h[3].name,
        ),
        compare_pair(
            "rnatr_target_constrained_periodic_parameters",
            outputs_11h[4],
            PROJECT_ROOT / "results/11_periodic_refinement" / RUN_ID / outputs_11h[4].name,
        ),
    ]
    write_comparison(COMPARISON_DIR / "15A5_11h_comparison.tsv", compare_11h)
    require_comparisons(compare_11h, "15A5/11h")
    if data_rows(outputs_11h[0], "tsv") != 49793:
        raise RuntimeError("11h expected 49,793 rows")
    ...
    require_comparisons([caller_compare0], "15A6 caller/reference")
    ...
    write_comparison(COMPARISON_DIR / "15A6_caller_comparison.tsv", caller_comparisons)
    require_comparisons(caller_comparisons, "15A6 caller determinism")
    ...
    if PACKAGE_FINAL.exists() and not PACKAGE_PART.exists():
        package_outputs_final = [PACKAGE_FINAL / path.name for path in package_outputs]
        materializer_record = {
            "stage": "15A7_reference_materializer",
            "stage_version": STAGE_VERSION,
            "source_stage_version": STAGE_VERSION,
            "execution_mode": "RESUME_FINAL_PACKAGE",
            "started_utc": ".",
            "finished_utc": ".",
    ...
    validator_rows.append(
        run_package_validator(
            package_work,
            "part" if package_work == PACKAGE_PART else "final_resume",
        )
    )
    validator_elapsed = time.perf_counter() - validator_started
    validator_timing_record = {
        "stage": "15A8_frozen_validators",
        "stage_version": STAGE_VERSION,
        "source_stage_version": STAGE_VERSION,
        "execution_mode": "EXECUTED_V013",
        "started_utc": ".",
    ...
    timing_records.append(validator_timing_record)

    package_rows, package_exact = package_comparison(package_work)
    require_comparisons(package_rows, "15A7 package")
    ...
    if package_work == PACKAGE_PART:
        ...
        try:
            ...
            os.close(parent_fd)
        final_validator_started = time.perf_counter()
        validator_rows.append(
            run_package_validator(PACKAGE_FINAL, "final_after_atomic_rename")
        )
        validator_timing_record["elapsed_seconds"] = (
            float(validator_timing_record["elapsed_seconds"])
            + time.perf_counter()
            - final_validator_started
        )
        validator_timing_record["finished_utc"] = utc_now()
    ...
    for table, expected in EXPECTED_COUNTS.items():
        ...
        if observed != expected:
            raise RuntimeError(
                f"final package row mismatch {table}: {observed} != {expected}"
            )
    ...
    production_stage_names = {
        ...
        "15A6_caller_production",
        "15A7_reference_materializer",
        "15A8_frozen_validators",
    }
    production_seconds = sum(
        float(record.get("elapsed_seconds", 0.0))
        for record in timing_records
        if bool(record.get("production_component"))
        and record.get("stage") in production_stage_names
    )
    v013_executed_seconds = sum(
        float(record.get("elapsed_seconds", 0.0))
        for record in timing_records
        if record.get("execution_mode") == "EXECUTED_V013"
        and bool(record.get("production_component"))
    )
    factor = 5312696 / 100000
    ...
    verify_active_11f_11h()
    snapshot_active_periodic_outputs("after")
    ...
    final_qc_rows: list[tuple[str, object]] = [
        ...
        ("stage15a4_failure_v012", "PROMOTED_CALLER_REQUIRES_FRESH_11H_PRIOR_NOT_PRODUCED"),
        ("stage15a4_fix_v013", "AMEND_REFERENCE_GRAPH_WITH_VALIDATED_11F_AND_11H"),
        ("stage15a_reference_graph", "11b>11d3>11e>11f>11h>native_v041>materializer_v012>schema_v042"),
        ("v012_upstream_resume_mode", "VERIFIED_BYTE_EXACT_WITH_CARRIED_TIMING"),
        ...
        ("stage15a7_reference_materializer", material_qc.get("audit_status", ".")),
        ("stage15a7_package_exact_logical_parity", str(package_exact).lower()),
        ...
        ("ssot_modified", "false"),
        ("full_5_31m_run_started", "false"),
        ("reference_bam_to_final_composed_seconds", f"{production_seconds:.6f}"),
        ("v013_executed_production_seconds", f"{v013_executed_seconds:.6f}"),
    ...
    output_tar = Path.home() / "Downloads/rnatr_stage15a_reference_100k_output_v0.1.3.tar.gz"
