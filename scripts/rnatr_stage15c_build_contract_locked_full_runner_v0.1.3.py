...
BUILDER_VERSION = "rnatr_stage15c_build_contract_locked_full_runner_v0.1.3"
...
BUILD_META_ROOT = PROJECT_ROOT / "metadata/stage15c/contract_locked_full_runner_v0.1.3"
BUILD_QC_ROOT = (
    PROJECT_ROOT / "qc/15_stage15c_contract_locked_runner_build"
    / ANALYSIS_RUN_ID_EXPECTED / "v0.1.3"
)
DOC_PATH = PROJECT_ROOT / "docs/stage15c/RNA_TR_Scout_contract_driven_full_runner_build_v0.1.3.md"
BUILDER_INSTALL = PROJECT_ROOT / "scripts/rnatr_stage15c_build_contract_locked_full_runner_v0.1.3.py"
RUNNER_INSTALL = PROJECT_ROOT / "scripts/rnatr_stage15c_run_full5312696_bam_to_final_v0.1.3.py"
LOCK_INSTALL = BUILD_META_ROOT / "rnatr_stage15c_full_runner_lock_contract_v0.1.3.json"
RUNNER_DOWNLOAD = DOWNLOADS / "rnatr_stage15c_run_full5312696_bam_to_final_v013.py"
SUCCESS_BUNDLE = DOWNLOADS / "rnatr_stage15c_contract_locked_full_runner_build_v0.1.3.tar.gz"
FAILURE_BUNDLE = DOWNLOADS / "rnatr_stage15c_contract_locked_full_runner_build_v0.1.3_failure.tar.gz"
...
def transform_template(source: str, c: dict[str, Any], lock_sha256: str) -> str:
    ...
    text = replace_once(text,
        ...
        'VERSION = "rnatr_stage15c_full5312696_bam_to_final_v0.1.3"', 'version')
    text = replace_once(text,
    ...
    text = text.replace('RNA_TR_Scout_fullscale_runner_execution_contract_v0.1.0.md', 'RNA_TR_Scout_fullscale_runner_execution_contract_v0.1.3.md')
    text = text.replace('rnatr_stage15c_run_full5312696_bam_to_final_v0.1.0.py', 'rnatr_stage15c_run_full5312696_bam_to_final_v0.1.3.py')
    text = text.replace('rnatr_stage15c_fullscale_runner_preflight_v0.1.0.tar.gz', 'rnatr_stage15c_fullscale_runner_preflight_v0.1.3.tar.gz')
    text = text.replace('rnatr_stage15c_full_empirical_run_v0.1.0.tar.gz', 'rnatr_stage15c_full_empirical_run_v0.1.3.tar.gz')
    text = text.replace('rnatr_stage15c_full_empirical_run_v0.1.0_failure.tar.gz', 'rnatr_stage15c_full_empirical_run_v0.1.3_failure.tar.gz')
    text = text.replace('rnatr_stage15c_fullscale_runner_preflight_v0.1.0', 'rnatr_stage15c_fullscale_runner_preflight_v0.1.3')
    text = text.replace('rnatr_stage15c_full_empirical_run_failure_v0.1.0', 'rnatr_stage15c_full_empirical_run_failure_v0.1.3')
    text = text.replace('rnatr_stage15c_full_empirical_run_v0.1.0', 'rnatr_stage15c_full_empirical_run_v0.1.3')
...
def generated_runner_stage15c_evidence_binding_test(path: Path, c: dict[str, Any]) -> dict[str, Any]:
    """Execute only the generated runner's Stage15C evidence verifier.

    This reads the validated contract/QC/resource/input-binding TSVs and performs
    no BAM/FASTQ hashing, partitioning, caller/materializer work, SSOT update,
    active-path modification, or 5.31M execution.
    """
    module_name = "rnatr_stage15c_generated_runner_evidence_selftest"
    spec = importlib.util.spec_from_file_location(module_name, path)
    if spec is None or spec.loader is None:
        raise BuildError(f"cannot import generated runner for evidence self-test: {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    try:
        spec.loader.exec_module(module)
        observed = module.verify_stage15c_144_evidence()
    finally:
        sys.modules.pop(module_name, None)

    expected = {
        "validated_shards": c["read_coherent_shards"],
        "validated_concurrency": c["active_shard_concurrency"],
        "caller_workers_per_shard": c["caller_workers_per_shard"],
        "validator_workers": c["validator_workers"],
        "sort_buffer": c["validator_sort_buffer"],
        "post_11b_max_candidate_rows_per_shard": c["post_11b_max_candidate_rows_per_shard"],
    }
    mismatches = {
    ...
    if mismatches:
        raise BuildError(
            "generated runner Stage15C evidence self-test mismatch: "
            + repr(mismatches)
        )
    return {
        "test": "generated_runner_real_stage15c_evidence_binding",
        "status": "PASS",
        "detail": (
            f"shards={observed['validated_shards']};"
            f"concurrency={observed['validated_concurrency']};"
            f"hard_max={observed['post_11b_max_candidate_rows_per_shard']};"
            f"projection_minutes={observed['validated_projection_minutes']};"
            f"projected_memory_fraction={observed['validated_projected_memory_fraction']}"
        ),
    }
...
def build() -> int:
    ...
    package = work / "rnatr_stage15c_contract_locked_full_runner_build_v0.1.3"
    package.mkdir()
    try:
        ...
        dynamic.append(generated_runner_stage15c_evidence_binding_test(runner_path, c))
        ...
        qc_metrics = [
            ...
            ("generated_runner_render_compile_test", "PASS"),
            ("generated_runner_real_evidence_binding_test", "PASS"),
            ...
            ("audit_status", "PASS"),
            ("next_gate", "RUN_V0.1.3_PREFLIGHT_ONLY_AND_UPLOAD_BUNDLE_FOR_PRO_REVIEW"),
            ("elapsed_seconds", f"{time.time() - started:.6f}"),
        ]
        write_metrics(package / "contract_locked_runner_build.qc.tsv", qc_metrics)

        doc = f'''# RNA-TR-Scout Stage 15C contract-driven full runner build v0.1.3\n\nThis build changes the development control plane, not the scientific core. Builder v0.1.3 is a generated-runner template-syntax amendment. v0.1.2 stopped safely at generated-runner compile before publication; v0.1.3 fixes the render boundary and keeps compile/static/dynamic/real-evidence checks before artifact publication. Builder v0.1.1 correctly bound the builder itself to the resource-model TSV schema, but the generated v0.1.1 runner still embedded the old top-level-QC field names and therefore stopped safely during preflight. v0.1.2 fixes the generated verifier, cross-checks resource-model and top-level-QC values, and executes the generated verifier against the real validated evidence before artifact publication. The validated 144-shard scientific/execution contract is unchanged.\n\nThe sole execution architecture source is the validated Stage 15C 144-shard contract (`{ARCH_CONTRACT_SHA256}`). The generated runner cannot choose a different shard count or full analysis run ID. A post-11b candidate-load hard gate of {c["post_11b_max_candidate_rows_per_shard"]:,} rows/shard is mandatory and is statically required to execute before candidate extraction and before caller/materializer.\n\nThe prior v0.1.0 runner (`{KNOWN_BAD_TEMPLATE_SHA256}`) is retained as failure provenance and is rejected by the new auditor. It is used only as an implementation template from which contract-controlled substitutions are made.\n\nThe v0.1.3 generated runner is intentionally execution-locked. Only `--preflight` is authorized. Full execution requires a new version after Pro review of the v0.1.3 preflight bundle. Active pipeline, SSOT, core schema, caller, materializer, accepted 500k results, and full BAM/FASTQ are not modified by this build.\n'''
        atomic_write(package / "RNA_TR_Scout_contract_driven_full_runner_build_v0.1.3.md", doc.encode("utf-8"), 0o644)
        shutil.copy2(Path(__file__).resolve(), package / "rnatr_stage15c_build_contract_locked_full_runner_v013.py")
        ...
        install_statuses = {
            ...
            "doc": install_exact_bytes((package / "RNA_TR_Scout_contract_driven_full_runner_build_v0.1.3.md").read_bytes(), DOC_PATH, 0o644),
        }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build and audit the contract-locked Stage15C full runner v0.1.3 (generated-runner template-syntax amendment v0.1.3)")
    parser.parse_args()
    try:
        return build()
    except Exception as exc:
