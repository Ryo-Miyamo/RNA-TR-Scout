...
RESULT_ROOT = PROJECT_ROOT / "results/15_stage15a_bam_to_final" / RUN_ID / "v0.2.2_performance"
QC_ROOT = PROJECT_ROOT / "qc/15_stage15a_bam_to_final" / RUN_ID / "v0.2.2_performance"
...
V0201_QC_SHA256 = "c634e60d79b96f0bb4513593410b9f1dec005d2d080216d103819b674e23c909"
V021_QC = PROJECT_ROOT / "qc/15_stage15a_bam_to_final" / RUN_ID / "v0.2.1_performance/stage15a_performance_100k.qc.tsv"
V021_QC_SHA256 = "5d4d40beecd2326082b1a7656144a7fb904cb078664afdbd1aca9e0d4f1d26ce"
V021_RESULT = PROJECT_ROOT / "results/15_stage15a_bam_to_final" / RUN_ID / "v0.2.1_performance"
REFERENCE_QC_SHA256 = "c5f379afaa5d4f9b7d4eadfac833d430801918713506c5821bd8a404cf57059a"
SSOT_GUARDS = {
...
ACTIVE_GUARDS = {
    ...
    PROJECT_ROOT / "config/evidence_schema/v0.4.2/rnatr_v041_validate_package.py": "e978b109d094f665ec62387ffda35c81d0aa9e8156972069f18a1b0b6c49bba5",
    PROJECT_ROOT / "config/evidence_schema/v0.4.2/rnatr_v042_validate_flank_uniqueness.py": "039024835de2bc1f096e562eed69788ecad9e481575b1b8cd58241edf2e87ab5",
}
...
FAST_MOTIF_BUILDER = PROJECT_ROOT / "scripts/rnatr_stage15a_fast_motif_jobs_v0.2.2.py"
FAST_MOTIF_BUILDER_SHA256 = "4dd972027c8b906703fa00c19f2981a8455a1ad9fb2bcc863321505c6619a5f2"
PARALLEL_PACKAGE_VALIDATOR = PROJECT_ROOT / "scripts/rnatr_stage15a_validate_package_parallel_v0.2.2.py"
PARALLEL_PACKAGE_VALIDATOR_SHA256 = "f3095a7f2af3099e8d960ebfa6d9b5380ba5e384d64122a386d6de49731ff9e2"
...
def verify_contract() -> dict[Path, str]:
    for path, expected in (
        (REFERENCE_QC, REFERENCE_QC_SHA256),
        (V0201_QC, V0201_QC_SHA256),
        (V021_QC, V021_QC_SHA256),
    ):
        ensure_file(path)
        observed = sha256_file(path)
        if observed != expected:
            raise RuntimeError(f"baseline QC SHA mismatch: {path}: {observed} != {expected}")
    ...
    v021_required = {
        ...
        "ssot_modified": "false",
        "full_5_31m_run_started": "false",
        "next_gate": "REVIEW_SHARD_WORKER_MATRIX_PROFILE_VALIDATOR_AND_FUSE_11D3_11E",
    }
    for key, expected in v021_required.items():
        observed = v021.get(key)
        if observed != expected:
            raise RuntimeError(f"v0.2.1 gate mismatch {key}: {observed} != {expected}")
    observed_v021_seconds = float(v021["performance_candidate_bam_to_final_seconds"])
    if abs(observed_v021_seconds - V021_SECONDS) > 1e-6:
        raise RuntimeError(
            f"v0.2.1 timing mismatch: {observed_v021_seconds} != {V021_SECONDS}"
        )
    ...
    for path in (ANALYSIS_REGIONS, DISEASE_REGIONS):
        ensure_file(path)
    for index in range(12):
        ref_jobs = (
            V021_RESULT / "shards" / f"shard_{index:03d}" / "project"
            / "results/11_motif_jobs" / RUN_ID / "motif_scan_jobs.tsv.gz"
        )
        ensure_file(ref_jobs)
...
def setup_shard_files(shards: list[Shard]) -> None:
    ...
    for shard in shards:
        ...
        paths_env.write_text(
            "# Stage 15A v0.2.2 read-coherent shard; no shard BAI; never source for active runs.\n"
...
def partition_inputs(shards: list[Shard], candidate_fastq_source: Path) -> dict[str, object]:
    ...
    return {
        "stage": "partition_inputs_no_bai_no_postwrite_sha_fastq_gzip0",
...
def run_fast_shared_catalog_motif_jobs(shards: list[Shard]) -> tuple[float, dict[str, object]]:
    manifest = QC_ROOT / "stage15a_fast_motif_jobs.input.tsv"
    summary = QC_ROOT / "stage15a_fast_motif_jobs.per_shard.tsv"
    rows = []
    for shard in shards:
        rows.append({
            "shard": shard.name,
    ...
    log = LOG_ROOT / "15AP3_fast_shared_catalog_motif_jobs.log"
    timing = TIMING_ROOT / "15AP3_fast_shared_catalog_motif_jobs.time_v.txt"
    command = [
        sys.executable,
        str(FAST_MOTIF_BUILDER),
        "--analysis-regions", str(ANALYSIS_REGIONS),
        "--disease-regions", str(DISEASE_REGIONS),
        "--shard-manifest", str(manifest),
        "--summary", str(summary),
        "--workers", str(len(shards)),
    ]
    started = time.perf_counter()
    with log.open("w", encoding="utf-8") as handle:
        proc = subprocess.run(
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(f"fast motif-job preparation failed; log={log}\n{tail}")
    timing_values = parse_time_v(timing)
    return elapsed, {
        "stage": "15AP3_fast_shared_catalog_motif_jobs",
        "elapsed_seconds": elapsed,
        "exit_code": proc.returncode,
        "maximum_resident_set_kbytes": timing_values.get("Maximum resident set size (kbytes)", "."),
        "log": str(log),
...
def run_caller_materializer_pipeline(
    shards: list[Shard], caller_workers_per_shard: int
) -> tuple[float, list[dict[str, object]], list[dict[str, object]], float]:
    started = time.perf_counter()

    def one(shard: Shard) -> tuple[dict[str, object], dict[str, object]]:
        caller = run_timed(
    ...
    materializer_rows: list[dict[str, object]] = []
    with cf.ThreadPoolExecutor(max_workers=len(shards)) as pool:
        futures = {pool.submit(one, shard): shard.name for shard in shards}
        for future in cf.as_completed(futures):
            caller, materializer = future.result()
            caller_rows.append(caller)
            materializer_rows.append(materializer)
    wall = time.perf_counter() - started
    caller_rows.sort(key=lambda row: str(row["shard"]))
    materializer_rows.sort(key=lambda row: str(row["shard"]))
    write_dict_tsv(QC_ROOT / "15AP4_native_caller_no_legacy_audit.per_shard.tsv", caller_rows)
    write_dict_tsv(QC_ROOT / "15AP5_materializer_plain_shards.per_shard.tsv", materializer_rows)
    max_materializer = max(float(row["elapsed_seconds"]) for row in materializer_rows)
    write_metrics(
        QC_ROOT / "stage15a_caller_materializer_pipeline.qc.tsv",
        [
            ("stage_version", STAGE_VERSION),
            ("pipeline_wall_seconds", wall),
            ("max_caller_shard_seconds", max(float(row["elapsed_seconds"]) for row in caller_rows)),
            ("max_materializer_shard_seconds", max_materializer),
            ("standalone_maxima_sum_seconds", max(float(row["elapsed_seconds"]) for row in caller_rows) + max_materializer),
            ("shards", len(shards)),
            ("audit_status", "PASS"),
        ],
    )
    return wall, caller_rows, materializer_rows, max_materializer
...
def run_package_validator_prepublication() -> dict[str, object]:
    ...
    log.parent.mkdir(parents=True, exist_ok=True)
    command = [
        sys.executable, str(PARALLEL_PACKAGE_VALIDATOR),
        "--schema-dir", str(SCHEMA_DIR),
    ...
    log.write_text(proc.stdout + proc.stderr, encoding="utf-8")
    return {
        "validator": "rnatr_stage15a_validate_package_parallel_v0.2.2.py",
        "table": "PACKAGE",
        "elapsed_seconds": elapsed,
        "exit_code": proc.returncode,
        "observed_rows": ".",
...
def compare_fast_motif_jobs_to_v021(shards: list[Shard]) -> tuple[float, bool]:
    started = time.perf_counter()
    rows: list[dict[str, object]] = []
    all_equal = True
    for shard in shards:
        candidate = shard.jobs_path
        reference = (
            V021_RESULT / "shards" / shard.name / "project"
            / "results/11_motif_jobs" / RUN_ID / "motif_scan_jobs.tsv.gz"
        )
        candidate_rows = data_rows(candidate)
        reference_rows = data_rows(reference)
        candidate_sha = logical_sha256(candidate)
        reference_sha = logical_sha256(reference)
        header_equal = header_bytes(candidate) == header_bytes(reference)
        equal = candidate_rows == reference_rows and candidate_sha == reference_sha and header_equal
        all_equal = all_equal and equal
        rows.append({
            "shard": shard.name,
            "candidate_rows": candidate_rows,
            "reference_rows": reference_rows,
            "candidate_logical_sha256": candidate_sha,
            "reference_logical_sha256": reference_sha,
            "header_equal": str(header_equal).lower(),
            "logical_equal": str(equal).lower(),
            "candidate_path": str(candidate),
            "reference_path": str(reference),
        })
    write_dict_tsv(COMPARISON_ROOT / "stage15a_fast_motif_jobs_v021_comparison.tsv", rows)
    if not all_equal:
        failed = [row["shard"] for row in rows if row["logical_equal"] != "true"]
        raise RuntimeError("fast motif-job parity failed: " + ",".join(map(str, failed)))
    return time.perf_counter() - started, all_equal


def run_frozen_package_validator_postpublication() -> tuple[float, str]:
    (LOG_ROOT / "validators").mkdir(parents=True, exist_ok=True)
    log = LOG_ROOT / "validators/frozen_package_postpublication.log"
    started = time.perf_counter()
    proc = subprocess.run(
        [sys.executable, str(VALIDATOR_PACKAGE), "--package-dir", str(PACKAGE_FINAL)],
        text=True,
        capture_output=True,
    )
    elapsed = time.perf_counter() - started
    log.write_text(proc.stdout + proc.stderr, encoding="utf-8")
    status = "PASS" if proc.returncode == 0 and "RNATR_V042_PACKAGE_VALIDATION_PASS" in proc.stdout else "FAIL"
    if status != "PASS":
        raise RuntimeError(f"frozen post-publication package validator failed: {log}")
    write_metrics(
        QC_ROOT / "stage15a_frozen_package_validator_postpublication.qc.tsv",
        [
...
def validator_missing_artifact_failure_parity() -> str:
    (LOG_ROOT / "validators").mkdir(parents=True, exist_ok=True)
    fixture = QC_ROOT / "validator_negative_fixture_missing_read_evidence"
    fixture.mkdir(parents=True, exist_ok=False)
    for path in PACKAGE_FINAL.iterdir():
        if not path.is_file() or path.name in {"read_evidence.tsv", "read_evidence.tsv.gz"}:
            continue
        (fixture / path.name).symlink_to(path)
    candidate = subprocess.run(
        [sys.executable, str(PARALLEL_PACKAGE_VALIDATOR), "--schema-dir", str(SCHEMA_DIR), "--package-dir", str(fixture)],
        text=True,
        capture_output=True,
    )
    frozen = subprocess.run(
        [sys.executable, str(VALIDATOR_PACKAGE), "--package-dir", str(fixture)],
        text=True,
        capture_output=True,
    )
    (LOG_ROOT / "validators/negative_candidate.log").write_text(candidate.stdout + candidate.stderr, encoding="utf-8")
    (LOG_ROOT / "validators/negative_frozen.log").write_text(frozen.stdout + frozen.stderr, encoding="utf-8")
    status = "PASS" if candidate.returncode != 0 and frozen.returncode != 0 else "FAIL"
    write_metrics(
        QC_ROOT / "stage15a_parallel_validator_failure_parity.qc.tsv",
        [
            ("fixture", "MISSING_READ_EVIDENCE_PLAIN_AND_GZIP"),
            ("candidate_exit_code", candidate.returncode),
            ("frozen_exit_code", frozen.returncode),
            ("failure_parity", status),
        ],
    )
    if status != "PASS":
        raise RuntimeError("parallel/frozen validator missing-artifact failure parity failed")
    return status
...
def main() -> int:
    ...
    if args.shards != 12 or args.caller_workers_per_shard != 2:
        raise ValueError(
            "v0.2.2 is a frozen 12-shard x 2-caller-worker benchmark; "
            f"observed shards={args.shards}, caller_workers_per_shard={args.caller_workers_per_shard}"
        )
    ...
    if RESULT_ROOT.exists() or QC_ROOT.exists():
        raise RuntimeError(
            f"v0.2.2 performance root already exists; preserve it and review before rerun: {RESULT_ROOT}"
        )
    ...
    wall_11e, fast_11e_record = run_fast_shared_catalog_motif_jobs(shards)
    timing_rows.append({"stage": "15AP3_fast_shared_catalog_motif_jobs", "elapsed_seconds": wall_11e})
    write_dict_tsv(QC_ROOT / "15AP3_fast_shared_catalog_motif_jobs.tsv", [fast_11e_record])
    load_projection_job_counts_from_qc(shards)

    wall_caller_materializer, per_caller, per_materializer, max_materializer_wall = (
        run_caller_materializer_pipeline(shards, args.caller_workers_per_shard)
    )
    timing_rows.append({
        "stage": "15AP4_5_caller_materializer_pipeline",
        "elapsed_seconds": wall_caller_materializer,
    })
    load_caller_metrics_from_qc(shards)

    merge_wall, merge_plain_wall, gzip_wall, merge_rows = merge_packages(
        shards, materializer_wall=max_materializer_wall
    )
    ...
    fast_11e_comparison_seconds, fast_11e_parity = compare_fast_motif_jobs_to_v021(shards)
    print(f"fast_motif_jobs_v021_logical_parity\t{str(fast_11e_parity).lower()}")

    frozen_validator_seconds, frozen_validator_status = run_frozen_package_validator_postpublication()
    negative_validator_parity = validator_missing_artifact_failure_parity()
    print(
        f"frozen_package_validator_postpublication\t{frozen_validator_status}"
        f"\twall_seconds={frozen_validator_seconds:.3f}"
    )
    ...
    next_gate = (
        ...
        if hard_status == "PASS"
        else "PROFILE_PARTITION_CALLER_AND_IMPLEMENT_TRUE_11B_11D3_FUSION"
    )
    final_rows: list[tuple[str, object]] = [
        ("stage_version", STAGE_VERSION),
        ("run_id", RUN_ID),
        ("reference_v013_correctness", "PASS"),
        ("candidate_graph", "read_hash_partition_fastq_gzip0>parallel_11b>parallel_11d3>shared_catalog_fast_11e>pipelined_native_v041_and_materializer>parallel_global_merge>parallel_global_gzip>concurrent_generic_and_parallel_exact_component_package_validation>atomic_publish>post_timer_frozen_validation_and_full_development_audit"),
        ("shard_count", args.shards),
        ("caller_workers_per_shard", args.caller_workers_per_shard),
        ("v0201_baseline_seconds", V0201_SECONDS),
        ("v021_baseline_seconds", V021_SECONDS),
        ("v021_to_v022_speedup", V021_SECONDS / production_seconds),
        ("production_audit_level", "STANDARD_FINAL_VALIDATION_PLUS_POST_TIMER_FULL_DEVELOPMENT_AUDIT"),
        ("shard_bai_created", "false"),
        ("partition_fastq_gzip_level", 0),
        ("fast_shared_catalog_motif_jobs", "true"),
        ("fast_motif_jobs_v021_exact_logical_parity", str(fast_11e_parity).lower()),
        ("caller_materializer_pipeline", "true"),
        ...
        ("frozen_package_validator_postpublication", frozen_validator_status),
        ("parallel_validator_missing_artifact_failure_parity", negative_validator_parity),
        ("validator_execution", "CONCURRENT_GENERIC_PLUS_PARALLEL_FROZEN_COMPONENTS_BEFORE_PUBLICATION"),
        ...
        ("fast_motif_jobs_reference_comparison_seconds", fast_11e_comparison_seconds),
        ("frozen_package_validator_postpublication_seconds", frozen_validator_seconds),
