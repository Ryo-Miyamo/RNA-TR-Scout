...
BASELINE="${RNATR_11F_CALLS:-$PROJECT_ROOT/results/11_periodic_baseline/$RUN_ID/high_confidence_simple_periodic_calls.tsv.gz}"
...
OUTDIR="${RNATR_11H_OUTDIR:-$PROJECT_ROOT/results/11_periodic_refinement/$RUN_ID}"
QCDIR="${RNATR_11H_QCDIR:-$PROJECT_ROOT/qc/11_periodic_refinement/$RUN_ID}"
WORKDIR="${RNATR_11H_WORKDIR:-$PROJECT_ROOT/tmp/11_periodic_refinement/$RUN_ID}"
...
_parallel_ids = []
_parallel_inputs = []
for _projection_id in sorted(jobs):
    _job = jobs[_projection_id]
    _projection = projections.get(_projection_id)
    if _projection is None:
        raise RuntimeError(f"Missing selected projection: {_projection_id}")
    _read_id = _projection["read_id"]
    _sequence = reads.get(_read_id)
    if _sequence is None:
        raise RuntimeError(f"Missing FASTQ read: {_read_id}")
    _read_length = len(_sequence)
    _motif = _job["_motif"]
    _search_start, _search_end, _search_scope = choose_search_interval(_projection, _read_length)
    if _search_start is None or _search_end is None:
        _search_sequence = ""
    else:
        _search_start = max(0, min(_search_start, _read_length))
        _search_end = max(_search_start, min(_search_end, _read_length))
        _search_sequence = _sequence[_search_start:_search_end]
    _parallel_ids.append(_projection_id)
    _parallel_inputs.append((_search_sequence, _motif))
...
sed -n '1,21p' "$TOP_CALLS" | column -ts $'\t'
