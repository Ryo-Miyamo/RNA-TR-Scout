...
INSTALLER_VERSION="rnatr_ssot_installer_v0.1.1"
PROJECT_ROOT="/mnt/intelssd/rnatr_project"
SSOT_ROOT="$PROJECT_ROOT/metadata/ssot"
CLI="$SSOT_ROOT/rnatr_ssot.py"
DESIGN="$PROJECT_ROOT/docs/design/RNA_TR_Scout_SSOT_design_v0.1.1.md"
INSTALLED_SCRIPT="$PROJECT_ROOT/scripts/00_build_rnatr_ssot_db_v0.1.1.sh"
...
if [[ ! -f "$VALIDATOR" ]]; then
  mapfile -t candidates < <(
    find "$PROJECT_ROOT/config/evidence_schema/v0.3/patches/validator_v0.3.1"       -maxdepth 1 -type f -name '*0.3.1*.py' -print 2>/dev/null | sort
  )
  if (( ${#candidates[@]} == 1 )); then
    VALIDATOR="${candidates[0]}"
  else
    echo "ERROR: validator_v0.3.1 not uniquely resolved" >&2
    printf 'candidate\t%s\n' "${candidates[@]:-NONE}" >&2
    exit 2
  fi
fi
...
def validate_db(conn: sqlite3.Connection, project_root: Path) -> list[tuple[str, str, str]]:
    ...
    if row and row[0]:
        ...
        checks.append((
            "current_validator_is_v0.3.1",
            "PASS" if validator_name_ok else "FAIL",
            validator_path.name,
        ))
        validator_sha = sha256_file(validator_path) if validator_exists else "."
        checks.append((
            "current_validator_sha256_recorded",
            "PASS" if validator_exists and len(validator_sha) == 64 else "FAIL",
            validator_sha,
        ))
    else:
        checks.append(("current_validator_exists", "FAIL", "not recorded"))
        checks.append(("current_validator_is_v0.3.1", "FAIL", "not recorded"))
...
if [[ "$SELF" != "$INSTALLED_SCRIPT" ]]; then
  if [[ -e "$INSTALLED_SCRIPT" ]]; then
    ...
    if [[ "$old_sha" != "$new_sha" ]]; then
      echo "ERROR: installed v0.1.1 script path contains different content: $INSTALLED_SCRIPT" >&2
      exit 2
    fi
  else
...
echo "===== RNATR SSOT INSTALL / MIGRATION v0.1.1 ====="
echo "installer version:       $INSTALLER_VERSION"
echo "project root:            $PROJECT_ROOT"
echo "legacy tracker policy:   READ ONLY"
echo "validator:               $VALIDATOR"
echo "validator SHA-256:       $(sha256sum "$VALIDATOR" | awk '{print $1}')"
...
echo
echo "===== CURRENT ALGORITHM CONTRACT ====="
python "$CLI" --project-root "$PROJECT_ROOT" show algorithm
...
stat -c '%n\t%s bytes'   "$CLI"   "$SSOT_ROOT/rnatr_ssot.sqlite"   "$SSOT_ROOT/CURRENT_STATE.md"   "$DESIGN"   "$INSTALLED_SCRIPT"
