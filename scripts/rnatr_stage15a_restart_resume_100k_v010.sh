...
META_ROOT="$PROJECT_ROOT/metadata/stage15a/v0.2.3/restart_resume_100k"
...
SELF_DST="$SCRIPT_ROOT/rnatr_stage15a_restart_resume_100k_v010.sh"
AUDITOR_DST="$SCRIPT_ROOT/rnatr_stage15a_restart_resume_100k_v0.1.0.py"
DOC_DST="$DOC_ROOT/RNA_TR_Scout_Stage15A_restart_resume_100k_v0.1.0.md"
CONSOLE="$DOWNLOADS/rnatr_stage15a_restart_resume_100k_v0.1.0.console.log"
SUCCESS_BUNDLE="$DOWNLOADS/rnatr_stage15a_restart_resume_100k_output_v0.1.0.tar.gz"
FAILURE_BUNDLE="$DOWNLOADS/rnatr_stage15a_restart_resume_100k_failure_v0.1.0.tar.gz"
TMP_ROOT="$(mktemp -d)"
AUDITOR_SRC="$TMP_ROOT/rnatr_stage15a_restart_resume_100k_v0.1.0.py"
DOC_SRC="$TMP_ROOT/RNA_TR_Scout_Stage15A_restart_resume_100k_v0.1.0.md"
...
install_exact() {
    ...
    if [[ -e "$destination" ]]; then
        ...
        if [[ "$source_sha" != "$destination_sha" ]]; then
            echo "ERROR: versioned destination exists with a different SHA: $destination" >&2
            return 1
        fi
...
pack_bundle() {
    ...
    rm -rf "$package_root"
    mkdir -p "$package_root/qc" "$package_root/scripts" "$package_root/docs" "$package_root/selected" "$package_root/console"
    if [[ -d "$QC_ROOT" ]]; then
        cp -a "$QC_ROOT"/. "$package_root/qc/" 2>/dev/null || true
    fi
    cp -a "$AUDITOR_DST" "$package_root/scripts/" 2>/dev/null || true
    cp -a "$SELF_DST" "$package_root/scripts/" 2>/dev/null || true
    cp -a "$DOC_DST" "$package_root/docs/" 2>/dev/null || true
    cp -a "$CONSOLE" "$package_root/console/" 2>/dev/null || true
    if [[ -d "$RESULT_ROOT/package_resume" ]]; then
        find "$RESULT_ROOT/package_resume" -maxdepth 1 -type f \
          \( -name '*manifest*' -o -name '*qc*' \) \
          -exec cp -a {} "$package_root/selected/" \; 2>/dev/null || true
    fi
    if [[ -f "$RESULT_ROOT/checkpoints/checkpoint_manifest.tsv" ]]; then
        cp -a "$RESULT_ROOT/checkpoints/checkpoint_manifest.tsv" "$package_root/selected/"
    fi
...
mkdir -p "$SCRIPT_ROOT" "$DOC_ROOT" "$META_ROOT" "$DOWNLOADS"

if ! "$PYTHON" -m py_compile "$AUDITOR_SRC"; then
    echo "ERROR: generated Python auditor does not compile" >&2
    exit 2
fi

if ! install_exact "$SELF_SOURCE" "$SELF_DST" 0755; then
    exit 2
fi
if ! install_exact "$AUDITOR_SRC" "$AUDITOR_DST" 0755; then
    exit 2
fi
if ! install_exact "$DOC_SRC" "$DOC_DST" 0644; then
    exit 2
fi

{
    echo -e "component\tpath\tsha256"
    echo -e "installer\t$SELF_DST\t$(sha256sum "$SELF_DST" | awk '{print $1}')"
    echo -e "auditor\t$AUDITOR_DST\t$(sha256sum "$AUDITOR_DST" | awk '{print $1}')"
    echo -e "design\t$DOC_DST\t$(sha256sum "$DOC_DST" | awk '{print $1}')"
} > "$META_ROOT/installation_manifest.tsv"

rm -f "$CONSOLE" "$SUCCESS_BUNDLE" "$SUCCESS_BUNDLE.sha256" "$FAILURE_BUNDLE" "$FAILURE_BUNDLE.sha256"

overall_rc=0
{
    echo "===== Stage 15A restart/resume 100k v0.1.0 ====="
    echo "active pipeline switch: PROHIBITED"
    echo "SSOT update:            PROHIBITED"
    echo "full 5.31M run:         PROHIBITED"
    echo
    set +e
    "$PYTHON" "$AUDITOR_DST" --prepare
    prepare_rc=$?
    set -e
    if [[ "$prepare_rc" -ne 75 ]]; then
        echo "ERROR: prepare phase returned $prepare_rc; expected intentional exit 75" >&2
        exit 2
    fi
    echo "prepare_phase\tEXPECTED_INTENTIONAL_STOP_75"
    echo
    "$PYTHON" "$AUDITOR_DST" --resume
    echo
    before_noop="$(sha256sum "$RESULT_ROOT/package_resume/package_manifest.tsv" | awk '{print $1}')"
    "$PYTHON" "$AUDITOR_DST" --resume
    after_noop="$(sha256sum "$RESULT_ROOT/package_resume/package_manifest.tsv" | awk '{print $1}')"
    if [[ "$before_noop" != "$after_noop" ]]; then
        echo "ERROR: no-op resume changed package manifest" >&2
        exit 2
    fi
    echo "noop_resume_package_manifest_unchanged\ttrue"
} 2>&1 | tee "$CONSOLE" || overall_rc=${PIPESTATUS[0]}

if [[ "$overall_rc" -eq 0 ]]; then
    pack_bundle "$SUCCESS_BUNDLE"
    echo
    echo "STAGE15A_RESTART_RESUME_100K_COMPLETE"
    echo "Upload:"
    echo "$SUCCESS_BUNDLE"
    echo "$SUCCESS_BUNDLE.sha256"
    exit 0
fi

pack_bundle "$FAILURE_BUNDLE"
echo
echo "ERROR: Stage 15A restart/resume audit failed (exit $overall_rc)." >&2
echo "Upload:" >&2
