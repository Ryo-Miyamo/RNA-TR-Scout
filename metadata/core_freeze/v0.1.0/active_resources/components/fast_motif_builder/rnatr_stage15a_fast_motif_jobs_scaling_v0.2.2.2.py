...
STAGE_VERSION = "rnatr_stage15a_fast_motif_jobs_scaling_v0.2.2.2"
...
def main() -> int:
    ...
    expected_projection_rows = sum(int(task["expected_rows"]) for task in tasks)
    expected_projection_reads = sum(int(task["expected_reads"]) for task in tasks)
    observed_projection_rows = sum(int(row["projection_rows"]) for row in rows)
    observed_projection_reads = sum(int(row["projection_reads"]) for row in rows)
    if observed_projection_rows != expected_projection_rows:
        raise SystemExit(
            f"aggregate projection row mismatch: "
            f"{observed_projection_rows} != {expected_projection_rows}"
        )
    if observed_projection_reads != expected_projection_reads:
        raise SystemExit(
            f"aggregate projection read mismatch: "
            f"{observed_projection_reads} != {expected_projection_reads}"
        )
    ...
    print(f"expected_projection_rows\t{expected_projection_rows}")
    print(f"observed_projection_rows\t{observed_projection_rows}")
    print(f"expected_projection_reads\t{expected_projection_reads}")
    print(f"observed_projection_reads\t{observed_projection_reads}")
