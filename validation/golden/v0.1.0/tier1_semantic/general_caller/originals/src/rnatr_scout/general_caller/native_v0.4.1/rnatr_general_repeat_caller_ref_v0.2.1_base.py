...
CALLER_VERSION='rnatr_general_repeat_caller_ref_v0.2.1'
...
def _ordered_orientations(motif:str):
    # Deterministic exact-tie contract:
    # preserve existing scoring/ranking; when motif and reverse-complement
    # are otherwise exactly tied, prefer the input/canonical orientation.
    motif=motif.upper()
    rc=revcomp(motif)
    return (motif,) if rc==motif else (motif,rc)
...
def _periodic_agreement(segment:str,motif:str):
    ...
    best=(-1.0,motif)
    for oriented in _ordered_orientations(motif):
        p=len(oriented)
        for phase in range(p):
...
def _best_oriented_alignment(seq:str,canon:str)->Alignment:
    best=None
    for oriented in _ordered_orientations(canon):
